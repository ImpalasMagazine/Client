...
const CompleteVehicleForm = ({data}) => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://temp-impalas-server.herokuapp.com/images/${data.img1}`)
    .then(res => {
      setImg1(res.data)
    }, err => {console.log(err)});
    axios.get(`https://temp-impalas-server.herokuapp.com/images/${data.img2}`)
    .then(res => {
      setImg2(res.data)
    }, err => {console.log(err)});}
  ...
  const accept = (e) => {
    setLoaded(false);
    setRejection(false);
    axios.post("https://temp-impalas-server.herokuapp.com/admin/requests/vehicles/accepted", {id:data._id,invoice:invoice},{
      headers: {
        "auth-token": token
      }
    }).then(res => {
  ...
  const reject = (e) => {
    setLoaded(false);
    axios.post("https://temp-impalas-server.herokuapp.com/admin/requests/vehicles/denied", {id:data._id,message:rejectMessage},{
      headers: {
        "auth-token": token
      }
    }).then(res => {
